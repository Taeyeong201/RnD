...
int main() {
...

	{
		...
		MsQuicRegistration* QUICRegist = new MsQuicRegistration();
		if (!QUICRegist->IsValid()) {
			std::cout << "Registration Failed" << std::endl;
			return -1;
		}
		MsQuicAlpn Alpn("cloudein");
		...
		Settings.GetGlobal();
		Settings.SetIdleTimeoutMs(50000);
		...
		if (QUIC_FAILED(Status = connection->Start(ClientConfiguration, "192.168.0.201", 12155))) {
